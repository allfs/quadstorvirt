...
int
do_connect2(int sockfd, struct sockaddr *addr, socklen_t len, int timeout)
{
	int status;
	struct timeval tv;
	...
	if (status < 0) {
		...
		while (ret <= 0 && timeout > 0)
		{
			...
			ret = select(FD_SETSIZE, NULL, &wfd, NULL, &tv);
...
struct tl_comm *
tl_msg_remote_connection(char *host, char *local, uint16_t port, int timeout)
{
	struct tl_comm *tl_comm;
	int sockfd = -1;
	int status = 0;
	int opt = 1;
	in_addr_t ipaddr, local_ipaddr = 0;
	...
	if (local && local[0]) {
		local_ipaddr = inet_addr(local);
		if (local_ipaddr == INADDR_NONE)
			return NULL;
	}
	...
	in_addr.sin_addr.s_addr = local_ipaddr;
	if (bind(sockfd, (struct sockaddr *)&in_addr, sizeof(in_addr)) == -1) {
		DEBUG_WARN("Cannot bind to addr %s\n", local);
		free(tl_comm);
		close(sockfd);
		return NULL;
	}
...
skip_bind: 
	...
	if (do_connect2(sockfd, (struct sockaddr *)&in_addr, sizeof(in_addr), timeout) < 0) {
		free(tl_comm);
		close(sockfd);
		return NULL;
	}

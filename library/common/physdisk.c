...
parse_disk(FILE *fp, struct d_list *dlist)
{
	...
	if (fscanf(fp, "dedupe_blocks: %"PRIu64"\n", &disk->dedupe_blocks) != 1) {
		DEBUG_ERR("Unable to get dedupe stats of disk\n");
		free(disk);
		return -1;
	}

	if (fscanf(fp, "total_blocks: %"PRIu64"\n", &disk->total_blocks) != 1) {
		DEBUG_ERR("Unable to get dedupe stats of disk\n");
		free(disk);
		return -1;
	}

	if (fscanf(fp, "uncompressed_size: %"PRIu64"\n", &disk->uncompressed_size) != 1) {
		DEBUG_ERR("Unable to get uncompressed stats of disk\n");
		free(disk);
		return -1;
	}

	if (fscanf(fp, "compressed_size: %"PRIu64"\n", &disk->compressed_size) != 1) {
		DEBUG_ERR("Unable to get compressed stats of disk\n");
		free(disk);
		return -1;
	}

	if (fscanf(fp, "compression_hits: %"PRIu64"\n", &disk->compression_hits) != 1) {
		DEBUG_ERR("Unable to get compressed stats of disk\n");
		free(disk);
		return -1;
	}
	...
	if (fscanf(fp, "initialized: %hhd\n", &disk->initialized) != 1) {
		DEBUG_ERR("Unable to get initialization status of disk\n");
		free(disk);
		return -1;
	}

	if (fscanf(fp, "ddmaster: %hhd\n", &disk->ddmaster) != 1) {
		DEBUG_ERR("Unable to get ddstatus flag of disk\n");
		free(disk);
		return -1;
	}

	if (fscanf(fp, "log_disk: %hhd\n", &disk->log_disk) != 1) {
		DEBUG_ERR("Unable to get log status flag of disk\n");
		free(disk);
		return -1;
	}

	if (fscanf(fp, "ha_disk: %hhd\n", &disk->ha_disk) != 1) {
		DEBUG_ERR("Unable to get ha status flag of disk\n");
		free(disk);
		return -1;
	}
	...
	if (fscanf(fp, "enable_comp: %hhd\n", &disk->enable_comp) != 1) {
		DEBUG_ERR("Unable to get comp status flag of disk\n");
		free(disk);
		return -1;
	}

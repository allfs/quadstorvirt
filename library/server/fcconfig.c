...
fc_rule_locate(struct fc_rule_spec *fc_rule_spec)
{
	...
	TAILQ_FOREACH(fc_rule, &fc_rule_list, q_entry) {
		if (fc_rule_spec->vdisk[0] && !fc_rule->vdisk)
			continue;
		if (!fc_rule_spec->vdisk[0] && fc_rule->vdisk)
			continue;
		if (fc_rule->vdisk && strcmp(fc_rule->vdisk->name, fc_rule_spec->vdisk))
			continue;
...
int
tl_server_remove_tdisk_fc_rules(struct tdisk_info *tdisk_info)
{
	int retval;
	struct fc_rule *fc_rule, *next;

	retval = sql_delete_tdisk_fc_rules(tdisk_info->target_id);
	if (retval != 0)
		return retval;

	TAILQ_FOREACH_SAFE(fc_rule, &fc_rule_list, q_entry, next) {
		if (fc_rule->vdisk != tdisk_info)
			continue;
...
tl_server_remove_fc_rule(struct tl_comm *comm, struct tl_msg *msg)
{
	...
	if (!fc_rule_spec_wwpn_valid(fc_rule_spec) && !fc_rule_spec->vdisk[0]) {
	...
	TAILQ_FOREACH_SAFE(fc_rule, &fc_rule_list, q_entry, next) {
		...
		if (fc_rule_spec->vdisk[0] && !fc_rule->vdisk)
			continue;
		if (fc_rule_spec->vdisk[0] && strcmp(fc_rule->vdisk->name, fc_rule_spec->vdisk))
			continue;
		retval = sql_delete_fc_rule(fc_rule);
		if (retval != 0) {
			tl_server_msg_failure2(comm, msg, "Cannot remove fc rule some/all specifications from db");
...
tl_server_add_fc_rule(struct tl_comm *comm, struct tl_msg *msg)
{
	...
	if (fc_rule_spec->vdisk[0]) {
		info = find_tdisk_by_name(fc_rule_spec->vdisk);
		if (!info) {
			tl_server_msg_failure2(comm, msg, "Cannot locate VDisk");
			return -1;
		}
	}
	else
...
tl_server_list_fc_rules(struct tl_comm *comm, struct tl_msg *msg)
{
	...
	TAILQ_FOREACH(fc_rule, &fc_rule_list, q_entry) {
		...
		if (fc_rule->vdisk)
			fprintf(fp, "wwpn: %s vdisk: %s rule: %d\n", wwpn, fc_rule->vdisk->name, fc_rule->rule);
		else
			fprintf(fp, "wwpn: %s vdisk: All rule: %d\n", wwpn, fc_rule->rule);
	}
	fclose(fp);
	tl_server_msg_success(comm, msg);
	return 0;

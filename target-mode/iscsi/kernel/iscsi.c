...
static void iscsi_scsi_queuecmnd(struct iscsi_cmnd *cmnd)
{
	...
	cmnd->ctio = NULL;
	device = ctio->ccb_h.tdisk;
	(*device->queue_ctio)(ctio->ccb_h.vdevice, ctio);
	set_cmnd_queued(cmnd);
	set_cmnd_waitio(cmnd);
	atomic_inc(&cmnd->conn->nr_busy_cmnds);
	return;
...
static void scsi_cmnd_start(struct iscsi_conn *conn, struct iscsi_cmnd *req)
{
	...
	valid = (*icbs.device_check_cmd)(req_hdr->scb[0]);
	if (valid != 0)
	{
	...
	switch (req_hdr->scb[0]) {
		...
		case COMPARE_AND_WRITE:
		{
			...
			retval = (*icbs.ctio_write_length)(ctio, ctio->ccb_h.tdisk, &num_blocks, &dxfer_len);
...
static int __cmnd_abort(struct iscsi_cmnd *cmnd, struct iscsi_session *from_session)
{
	...
	if (cmnd_waitio(cmnd)) {
		...
		task_found = (*icbs.device_istate_abort_task)(target->tdisk, abort_i_prt, abort_t_prt, TARGET_INT_ISCSI, cmnd_itt(cmnd));
		if (!task_found)
			return 0;
	}
...
static int target_reset(struct iscsi_cmnd *req, u32 lun, int all)
{
	...
	reset_t_prt[0] = t_prt;
	reset_t_prt[1] = 0;
	(*icbs.device_target_reset)(target->tdisk, reset_i_prt, reset_t_prt, TARGET_INT_ISCSI);
	return 0;
}

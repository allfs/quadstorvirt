...
print_usage(void)
{
	...
	fprintf(stdout, "fcconfig -a -v <vdisk>  -r <allow|disallow> -w <wwpn>\n");
	fprintf(stdout, "fcconfig -x -v <vdisk>  -w <wwpn>\n");
...
dump_fc_rule_list(void)
{
	...
	while ((fgets(buf, sizeof(buf), fp) != NULL)) {
		retval = sscanf(buf, "wwpn: %s vdisk: %s rule: %d\n", wwpn, vdisk, &rule);
		if (retval != 3) {
...
int main(int argc, char *argv[])
{
	...
	while ((c = getopt(argc, argv, "w:r:v:lax")) != -1) {
		...
		case 'r':
			...
			break;
		case 'v':
			strncpy(vdisk, optarg, sizeof(vdisk) - 1);
			break;
		case 'a':
			add = 1;
			break;
	...
	if (list) {
		...
		strcpy(fc_rule_spec.vdisk, vdisk);
		if (add) {

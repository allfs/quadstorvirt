...
int main()
{
	...
	char status[512];
	char name[512];
	char *cols[] = {"{ key: 'Name', sortable: true}", "Status", "Disks", "VDisks", "{ key: 'Details', allowHTML: true }", "{ key: 'Delete', label: ' ',  allowHTML: true }", NULL};
	...
	printf("<div style=\"float: right;\"><p style=\"font-size: x-small;\"><i>D: Dedupe Metadata L: Log Data</i></p></div>\n");
	...
	TAILQ_FOREACH(group_info, &group_list, q_entry) {
		...
		cgi_print_column_format("VDisks", "%d", group_info->tdisks);
		cgi_print_comma();
